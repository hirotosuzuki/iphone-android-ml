...
//====================
    ...
    private void initModel() {
        ...
        FirebaseCloudModelSource cloudSource =
            new FirebaseCloudModelSource.Builder("mobilenet")
        ...
        FirebaseLocalModelSource localSource =
            new FirebaseLocalModelSource.Builder("local_mobilenet")
        ...
        try {
            FirebaseModelOptions options = new FirebaseModelOptions.Builder()
                ...
                .setLocalModelName("local_mobilenet")
                .build();
...
//====================
        ...
        try {
            ...
            FirebaseModelInputOutputOptions inputOutputOptions =
                new FirebaseModelInputOutputOptions.Builder()
                    ...
                    .setOutputFormat(0, FirebaseModelDataType.BYTE, new int[]{1, 1001})
                    .build();

            //(7)モデルの入力データの生成
            byte[][][][] input = image2inputData(image, new Point(224, 224));
                ...
                .addOnSuccessListener(
                    new OnSuccessListener<FirebaseModelOutputs>() {
                        ...
                        public void onSuccess(FirebaseModelOutputs outputs) {
                            //検出結果の取得
                            byte[][] output = outputs.getOutput(0);

                            //outputsをHashMapに変換
                            Map<Integer, Integer> hashMap = new HashMap<>();
                            byte[] inArray = output[0];
                            ...
                            for (int i = 0; i < Math.min(3, entries.size()); i++) { //上位3件
                                ...
                                int accuracy = (int)((float)s.getValue()*100f/255f); //信頼度
    ...
    //(8)Bitmapを多次元配列に変換
    private byte[][][][] image2inputData(Bitmap image, Point size) {
        //BitmapのリサイズとRGBAのint配列への変換
        int[] rgbaData = cgImage2rgbaData(image, size);

        //RGBAのint配列をRGBのbyte配列に変換
        byte[][][][] inputData = new byte[1][size.y][size.x][3];
        for (int i = 0; i < rgbaData.length; i++) {
            inputData[0][i/size.x][i%size.x][0] = (byte)Color.red(rgbaData[i]);
            inputData[0][i/size.x][i%size.x][1] = (byte)Color.green(rgbaData[i]);
            inputData[0][i/size.x][i%size.x][2] = (byte)Color.blue(rgbaData[i]);
        }
        return inputData;
    }
